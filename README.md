...
**Code was written and tested using Python (version 3.14.1) and Jupyter Notebook (version 7.3.2)**
...
1. Copy `configs/templates/gam_template.yaml`
2. Edit file paths + column mappings
3. Run:

```bash
python run_gam.py --config path/to/your_gam.yaml
```

## Run Generic LMM

1. Copy `configs/templates/lmm_template.yaml`
2. Edit file paths + schema/covariates
3. Run:

```bash
python run_lmm.py --config path/to/your_lmm.yaml
```

## TB Experiment Shortcuts
...
TB defaults are stored in:

- `experiments/tb/configs/gam_tb.yaml`
- `experiments/tb/configs/lmm_tb.yaml`

## Converter Tools

Grouped under `tools/converters/`:

- `convert_mutation_style.py`
- `convert_snps_to_amino_acid.py`

Core converter functions live in `src/gam/converters/`.
