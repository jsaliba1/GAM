...
class FineAnalysisModel:
    ...
    def __init__(
        ...
        self.snp_matrix: pd.DataFrame | None = None
    ...
    def from_csv(
        ...
        drug_profiles = pd.read_csv(drug_file, index_col="UNIQUEID").replace(
            ["S", "R", "I"], [1, -1, 0]
        )
        ...
        for folder_name in self.folder_names:
            ...
            if self.run_num != 0 and not snps.empty:
                ...
                if repeat_genes:
                    ...
                    duplicate_idx = (
                        duplicate_snps.groupby(["MUTATION", "GENE"])["MUTATION"].count().index
                    )
